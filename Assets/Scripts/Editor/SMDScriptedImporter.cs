...
namespace SilentTools
{
	...
	public class SMDScriptedImporter : ScriptedImporter
	{
		...
		{
			Vector3 flippedRotation = new(rotation.x, -rotation.y, -rotation.z); // flip Y and Z axis for right->left handed conversion
			// convert XYZ to ZYX
		...
		public override void OnImportAsset(AssetImportContext ctx)
		{
			...
			string smdRawData = File.ReadAllText(ctx.assetPath);
			string[] smdLines = smdRawData.Split('\n');
			ParsingState state = ParsingState.None;
			var parsedNodes = new List<Node>();
			var parsedFrames = new List<SMDFrame>();
			List<string> curvePaths;

			foreach (string smdLine in smdLines)
			{
				string[] lineData = smdLine.Trim().Split(' ');
				...
				if (lineData.Length > 0)
				{
					...
					if (state == ParsingState.Nodes)
					{
						...
						parsedNodes.Add(
							new()
							{
								...
								name = lineData[1].Substring(1, lineData[1].Length - 2), parent = int.Parse(lineData[2]),
							}
						);
					}
					...
					if (state == ParsingState.Skeleton)
					{
						...
						if (lineData[0] == "time")
						{
							parsedFrames.Add(new() { time = int.Parse(lineData[1]), transforms = new() });
						}
						else
						{
							...
							if (currentFrame > 0)
							{
								parsedFrames[currentFrame - 1].transforms.Add(
									new() { node = int.Parse(lineData[0]), position = parseVector3(lineData[1], lineData[2], lineData[3]), rotation = parseVector3(lineData[4], lineData[5], lineData[6]) * Mathf.Rad2Deg }
								);
							}
						}
					}
			...
			curvePaths = new(parsedNodes.Count);
			...
			var animationCurves = new Dictionary<string, ClipTransformCurves>();
			...
			foreach (SMDFrame frame in frames)
			{
				...
				foreach (SMDTransform transform in frame.transforms)
				{
					...
					{
						animationCurves.Add(curvePaths[transform.node], new());
					}
