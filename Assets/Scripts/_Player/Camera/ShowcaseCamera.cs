 ...
 * ShowcaseCamera.cs
 * Created by: Ambrosia
 * Created on: 9/4/2020 (dd/mm/yy)
 * Created for: THIS IS JUST UNITY'S DEFAULT CAMERA CONTROLLER
 */
...
public class ShowcaseCamera : MonoBehaviour
{
	...
	void Update()
	{
		...
		{
			Vector2 mouseMovement = new(Input.GetAxis("Mouse X"), Input.GetAxis("Mouse Y") * (invertY ? 1 : -1));
	...
	class CameraState
	{
		...
		{
			t.eulerAngles = new(pitch, yaw, roll);
			t.position = new(x, y, z);
		}
	}
}
