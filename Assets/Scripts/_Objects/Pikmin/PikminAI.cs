...
public class PikminAI : MonoBehaviour, IHealth, IComparable, IInteraction
{
    ...
    [SerializeField]
    Transform _HeadBoneTransform;

    [SerializeField]
    VisualEffect _FireVFX;

    [SerializeField]
    VisualEffectAsset _AttackVFX;

    [Header("Behaviour")]
    [SerializeField]
    float _LatchNormalOffset = 0.5f;
    ...
    [Header("Idle")]
    [SerializeField]
    float _IdleTickRate = 0.3f;

    [Header("Object Avoidance")]
    [SerializeField]
    float _AvoidSphereSize = 0.5f;

    [SerializeField]
    float _PlayerPushScale = 30.0f;

    [SerializeField]
    float _PikminPushScale = 15.0f;

    [SerializeField]
    LayerMask _InteractableMask = 0;

    [SerializeField]
    LayerMask _RunTowardsMask = 0;

    [SerializeField]
    TrailRenderer _ThrowTrailRenderer;

    [SerializeField]
    LayerMask _PlayerAndPikminLayer = 0;
    ...
    [HideInInspector]
    public Transform _FormationPosition;

    [Header("Head Types")]
    [SerializeField]
    Transform _LeafSpawn;

    [SerializeField]
    PikminMaturity _StartingMaturity = PikminMaturity.Leaf;
    Animator _Animator;
    ...
    [SerializeField]
    Collider _TargetObjectCollider;

    [SerializeField]
    PikminIntention _Intention = PikminIntention.Idle;
    ...
    [Header("Suck Nectar")]
    [SerializeField]
    float _SuckNectarTimer;

    [SerializeField]
    Transform _NectarTransform;
    ...
    [Header("Stats")]
    [SerializeField]
    PikminStatSpecifier _CurrentStatSpecifier;
    ...
    [SerializeField]
    float _ColliderOriginRadius;

    [SerializeField]
    float _ColliderOriginHeight;
    ...
    void FixedUpdate()
    {
        ...
        if (
            _CurrentState == PikminStates.RunTowards && _Intention != PikminIntention.Attack
        ...
        if (!Latch_IsLatchedOntoObject() || _CurrentState == PikminStates.Push)
        {
            float newYRotation = Mathf.LerpAngle(
                _EulerAngles.y,
                _FaceDirectionAngle,
                _Data._RotationSpeed * Time.fixedDeltaTime
            );
    ...
    void HandleIdle()
    {
        ...
        Collider[] objects = Physics.OverlapSphere(
            _Transform.position,
            _Data._SearchRadius,
            _InteractableMask | _RunTowardsMask
        );

        foreach (Collider col in objects)
        {
            IPikminInteractable interactableComponent =
                col.GetComponentInParent<IPikminInteractable>();
            ...
            if (
                currentIntention == PikminIntention.Attack
            ...
            Vector3 direction = MathUtil.DirectionFromTo(
                _Transform.position,
    ...
    void HandlePush()
    {
        ...
        if (MathUtil.DistanceTo(_Transform.position, destination, false) <= 0.02f)
        {
            ...
            _FaceDirectionAngle = Quaternion
                .LookRotation(_Pushing.GetMovementDirection())
                .eulerAngles.y;
    ...
    void HandleSuckNectar()
    {
        ...
        if (_NectarTransform != null)
        {
            _FaceDirectionAngle = Quaternion
                .LookRotation(
                    MathUtil.DirectionFromTo(_Transform.position, _NectarTransform.position)
                )
                .eulerAngles.y;
    ...
    void HandleDeath()
    {
        ...
        {
            ParticleSystem soul = Instantiate(
                    _DeathParticle,
                    _Transform.position + Vector3.up * 1.25f,
        ...
        int totalPikmin =
            PikminStatsManager.GetTotalPikminInAllOnions()
            + PikminStatsManager.GetTotalPikminOnField();
    ...
    void HandleOnFire()
    {
        ...
        _FaceDirectionAngle = Mathf.LerpAngle(
            _FaceDirectionAngle,
            _WanderAngle,
            3.0f * Time.fixedDeltaTime
        );
    ...
    void MoveTowards(Vector3 position, bool stopEarly = true, bool rotateTowards = true)
    {
        ...
        if (delta != Vector3.zero && rotateTowards)
        {
            ...
            _FaceDirectionAngle = _InSquad
                ? Quaternion
                    .LookRotation(
                        MathUtil.DirectionFromTo(_Transform.position, _PlayerTransform.position)
                    )
                    .eulerAngles.y
        ...
        if (
            stopEarly
            && MathUtil.DistanceTo(_Transform.position, position, false) < _StoppingDistance
        )
        {
    ...
    void MoveTowardsTarget()
    {
        ...
        switch (_Intention)
        {
            case PikminIntention.Push when _Pushing != null:
                ...
                break;
            default:
                destination = ClosestPointOnTarget(
                    _TargetObject,
                    _TargetObjectCollider,
                    _Data._SearchRadius
                );
                break;
        }
    ...
    void RepelPikminAndPlayers()
    {
        ...
        foreach (Collider col in objects)
        {
            ...
            else if (col.CompareTag("Player"))
            {
                ...
                col.CompareTag("PikminInteract")
                && col.TryGetComponent(out IPikminCarry carry)
                && carry.IntentionType == PikminIntention.Carry
            )
            {
                Vector3 direction =
                    MathUtil.DirectionFromTo(col.transform.position, _Transform.position) * 2.0f;
                _AddedVelocity += _PlayerPushScale * Time.fixedDeltaTime * direction;
            }
        }
    }
        ...
        switch (_CurrentState)
        {
            ...
            case PikminStates.Thrown when col.CompareTag("PikminInteract"):
            {
                ...
                if (squish != null)
                {
                    ...
                    float spot = Vector3.Dot(
                        Vector3.up,
                        MathUtil.DirectionFromTo(_Transform.position, closestPoint, true)
                    );
            ...
            case PikminStates.RunTowards
                when _TargetObjectCollider == null
                    || _TargetObjectCollider != col
                    || _TargetObjectCollider.gameObject.layer == _RunTowardsMask:
                return;
            case PikminStates.RunTowards:
                _Intention = _TargetObjectCollider
                    .GetComponentInParent<IPikminInteractable>()
                    .IntentionType;
                CarryoutIntention();
                break;
            default:
    ...
    void MaintainLatch()
    {
        ...
        Vector3 directionFromPosToObj = MathUtil.DirectionFromTo(
            pos,
            _LatchedTransform.position,
            !useY
        );
    ...
    public void EndThrowHold()
    {
        ...
        _FaceDirectionAngle = Quaternion
            .LookRotation(MathUtil.DirectionFromTo(_PlayerTransform.position, _Transform.position))
            .eulerAngles.y;
    ...
    public void LatchOnto(Transform obj)
    {
        ...
        if (obj != null)
        {
            ...
            if (!IsGrounded())
            {
                ...
                Vector3 closestPosition = ClosestPointOnTarget(
                    _TargetObject,
                    _TargetObjectCollider
                );
                Vector3 dirToClosestPos = MathUtil.DirectionFromTo(
                    _Transform.position,
                    closestPosition,
                    true
                );
    ...
    public void ANIM_Attack()
    {
        ...
        _AttackVFXInstance.transform.position =
            _HeadBoneTransform.position - _HeadBoneTransform.forward / 5;
    ...
    {
        if (
            _CurrentMaturity == PikminMaturity.Size - 1
    ...
    {
        if (
            _CurrentState == PikminStates.Dead
            || _CurrentState == PikminStates.Squish
