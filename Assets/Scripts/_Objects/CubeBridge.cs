...
public class CubeBridge : MonoBehaviour, IPikminAttack, IHealth
{
	...
	[SerializeField] GameObject _EndsPiece;

	// Will be used for the Pikmin to attack
	[SerializeField] GameObject _AttackablePiece;
	...
	readonly List<PikminAI> _AttackingPikmin = new();

	// Stores all of the bridge pieces ([0] will be start ramp, [1] will be end ramp)
	readonly List<GameObject> _BridgePieces = new();
	...
	void Awake()
	{
		...
		startingRamp.transform.SetPositionAndRotation(
			_StartPoint.position + Vector3.up * _RampHeightOffset,
			Quaternion.Euler(
				lookingAtEnd.eulerAngles.x - _AngleOfRamp + _RotationOffset.x,
				lookingAtEnd.eulerAngles.y + _RotationOffset.y,
				lookingAtEnd.eulerAngles.z + _RotationOffset.z
			)
		);
		...
		endingRamp.transform.SetPositionAndRotation(
			_EndPoint.position + Vector3.up * _RampHeightOffset,
			Quaternion.Euler(
				lookingAtStart.eulerAngles.x - _AngleOfRamp + _RotationOffset.x,
				lookingAtStart.eulerAngles.y + _RotationOffset.y,
				lookingAtStart.eulerAngles.z + _RotationOffset.z
			)
		);
	...
	void OnDrawGizmos()
	{
		...
		Gizmos.DrawMesh(
			endMesh,
			_StartPoint.position + Vector3.up * rampHeightOffset,
		...
		Gizmos.DrawMesh(
			endMesh,
			_EndPoint.position + Vector3.up * rampHeightOffset,
		...
		for (int i = 0; i < stepsToFinish; i++)
		{
			...
			Gizmos.DrawMesh(
				pieceMesh,
				point + Vector3.up * (Mathf.Sin(_AngleOfRamp * Mathf.Deg2Rad) - _Midpiece.transform.localScale.y / 2),
				lookRotation,
				_Midpiece.transform.localScale
			);
		}
	...
	void Step()
	{
		...
		Vector3 nextPosition = _CurrentStepPos
		                       + Vector3.up * (Mathf.Sin(_AngleOfRamp * Mathf.Deg2Rad)
		                                       - _Midpiece.transform.localScale.y / 2);
